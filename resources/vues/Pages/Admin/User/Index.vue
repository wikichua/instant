<template>
    <authenticated-layout>
        <template #page-title>
            <inertia-link class="text-white text-sm uppercase hidden lg:inline-block font-semibold" :href="route('user')">
        ...
        <instant-content-card>
            ...
            <instant-datatable :models="models" :columns="columns" :actionsComponent="InstantActions">
                <template #header-action-slot>
                    <instant-create-link :href="route('user.create')" class="text-indigo-600 hover:text-indigo-900 inline-block" v-if="can.create" />
                </template>
            </instant-datatable>
...
<script>
    ...
    import InstantActions from '@/Pages/Admin/User/Actions'
    import InstantSearch from '@/Pages/Admin/User/Search'
